...
namespace ncml_module
{
  ...
  , _dimensions()
  , _variableValidator(this) // start it empty rather than copy to avoid ref counting errors...
  {
  ...
  void
  NetcdfElement::addVariableToValidateOnClose(libdap::BaseType* pNewVar, VariableElement* pVE)
  {
    _variableValidator.addVariableToValidate(pNewVar, pVE);
  }

  void
  NetcdfElement::setVariableGotValues(libdap::BaseType* pVarToValidate, bool removeEntry)
  {
  ...
  void
  NetcdfElement::VariableValueValidator::addVariableToValidate(libdap::BaseType* pNewVar, VariableElement* pVE)
  {
    VALID_PTR(pNewVar);
    VALID_PTR(pVE);
    {
      VVVEntry* pExisting = findEntryByLibdapVar(pNewVar);
      NCML_ASSERT_MSG(!pExisting, "NetcdfElement::VariableValueValidator::addVariableToValidate: var was already added!");
    }
  ...
  void
  NetcdfElement::VariableValueValidator::removeVariableToValidate(libdap::BaseType* pVarToRemove)
  {
  ...
  void
  NetcdfElement::VariableValueValidator::setVariableGotValues(libdap::BaseType* pVarToValidate)
  {
    VALID_PTR(pVarToValidate);
    VVVEntry* pEntry = findEntryByLibdapVar(pVarToValidate);
    NCML_ASSERT_MSG(pEntry, "NetcdfElement::VariableValueValidator::setVariableGotValues: expected to find the var name=" +
  ...
  bool
  NetcdfElement::VariableValueValidator::validate()
  {
    VVVEntries::iterator it;
    VVVEntries::iterator endIt = _entries.end();
    for (it = _entries.begin(); it != endIt;  ++it)
        ...
        if (!entry._pVarElt->checkGotValues())
          {
            THROW_NCML_PARSE_ERROR(_pParent->line(),
                "On closing the <netcdf> element, we found a new variable name=" +
                entry._pNewVar->name() +
                " that was added"
