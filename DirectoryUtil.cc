  ...
  */
      ...
      bool allowSymLinks/*=false*/)
  {
    if (!allowRelativePaths && hasRelativePath(origRootDir))
      {
        throw BESForbiddenError("can't use rootDir=" + origRootDir + " since it has a relative path (../)", __FILE__, __LINE__);
      }
  ...
  void
  DirectoryUtil::getListingForPathRecursive(const std::string& path,
           std::vector<FileInfo>* pRegularFiles,
           std::vector<FileInfo>* pDirectories)
  {
    ...
    for (vector<FileInfo>::const_iterator it = dirs.begin();
         it != dirs.end();
         ++it)
      {
        string subPath = canonicalPath + "/" + it->basename();
        BESDEBUG(_sDebugChannel, "DirectoryUtil: recursing down to directory subtree=\"" <<
            subPath << "\"..." << endl);
  ...
  void
  DirectoryUtil::getListingOfRegularFilesRecursive(const std::string& path,
  ...
  void
  DirectoryUtil::throwErrorForOpendirFail(const string& fullPath)
  {
    switch (errno)
      {
      case EACCES:
  ...
  bool
  DirectoryUtil::hasRelativePath(const std::string& path)
  {
    return (path.find("..") != string::npos);
  }

  void
  DirectoryUtil::removeTrailingSlashes(string& path)
  {
    if (!path.empty())
      {
  ...
  void
  DirectoryUtil::removePrecedingSlashes(std::string& path)
  {
    if (!path.empty())
      {
  ...
  void
  DirectoryUtil::printFileInfoList(const vector<FileInfo>& listing)
  {
  ...
  void
  DirectoryUtil::printFileInfoList(std::ostream& os, const vector<FileInfo>& listing)
  {
     for (vector<FileInfo>::const_iterator it = listing.begin();
          it != listing.end();
          ++it)
       {
         os << it->toString() << endl;
