...
namespace ncml_module
{
  ...
  MyBaseTypeFactory::makeVariable(const string& type, const std::string& name)
  {
    if (isArrayTemplate(type))
      {
        return makeArrayTemplateVariable(type, name);
      }
    else
      {
  ...
  std::auto_ptr<libdap::BaseType>
  MyBaseTypeFactory::makeArrayTemplateVariable(const string& type, const string& name)
  {
    BaseType* pNew = 0;
    if (type == "Array<Byte>")
      {
