...
VariableElement::VariableElement() :
		RCObjectInterface(), NCMLElement(0), _name(""), _type(""), _shape(""), _orgName(
				""), _shapeTokens(), _pNewlyCreatedVar(0), _gotValues(false)
{
}
...
void VariableElement::processEnd(NCMLParser& p)
{
	...
	if (isNewVariable() && !checkGotValues())
	{
		...
		_parser->getCurrentDataset()->addVariableToValidateOnClose(
				_pNewlyCreatedVar, this);
	}
	...
	BESDEBUG("ncml",
			"Variable scope now with name: " << ((pVar)?(pVar->name()):("<NULL>")) << endl);
}
...
void VariableElement::processExistingVariable(NCMLParser& p, BaseType* pVar)
{
	...
	if (!_type.empty()
			&& !p.typeCheckDAPVariable(*pVar,
					p.convertNcmlTypeToCanonicalType(_type)))
	{
		THROW_NCML_PARSE_ERROR(_parser->getParseLineNumber(),
				"Type Mismatch in variable element with name=" + _name
						+ " at scope=" + p.getScopeString() + " Expected type="
						+ _type + " but found variable with type="
						+ pVar->type_name()
...
void VariableElement::processRenameVariableDataWorker(NCMLParser& p,
		BaseType* pOrgVar)
{
...
void VariableElement::processRenameVariable(NCMLParser& p)
{
	...
	BESDEBUG("ncml",
			"Looking up the existence of a variable with name=" << _orgName << "..." <<endl);
	...
	if (!pOrgVar)
	{
		THROW_NCML_PARSE_ERROR(_parser->getParseLineNumber(),
				"Renaming variable failed for element=" + toString()
						...
						+ " exists at current parser scope="
						+ p.getScopeString());
	}
	...
	BESDEBUG("ncml",
			"Making sure new name=" << _name << " does not exist at this scope already..." << endl);
	...
	if (pExisting)
	{
		THROW_NCML_PARSE_ERROR(_parser->getParseLineNumber(),
				"Renaming variable failed for element=" + toString()
						...
						+ " already exists at current parser scope="
						+ p.getScopeString());
	}
	BESDEBUG("ncml",
	...
	BESDEBUG("ncml",
			"Renaming variable " << _orgName << " to " << _name << endl);
	...
	else
	{
		...
		auto_ptr<BaseType> pCopy = auto_ptr < BaseType
				> (pOrgVar->ptr_duplicate());
...
void VariableElement::processNewVariable(NCMLParser& p)
{
	...
	{
		THROW_NCML_PARSE_ERROR(_parser->getParseLineNumber(),
				"Must have non-empty variable@type when creating new variable="
						+ toString());
	}
...
void VariableElement::processNewStructure(NCMLParser& p)
{
	...
	auto_ptr<BaseType> pNewVar = MyBaseTypeFactory::makeVariable("Structure",
			_name);
	NCML_ASSERT_MSG(pNewVar.get(),
			"VariableElement::processNewStructure: factory failed to make a new Structure variable for name="
					+ _name);
...
void VariableElement::processNewArray(NCMLParser& p, const std::string& dapType)
{
	...
	auto_ptr<BaseType> pTemplateVar = MyBaseTypeFactory::makeVariable(dapType,
			_name);
	...
	for (unsigned int i = 0; i < _shapeTokens.size(); ++i)
	{
		...
		string dimName = (
				(isDimensionNumericConstant(_shapeTokens.at(i))) ?
						("") : (_shapeTokens.at(i)));
		BESDEBUG("ncml",
	...
	if (getProductOfDimensionSizes(p)
			> static_cast<unsigned int>(DODS_MAX_ARRAY)) // actually the call itself will throw...
	{
		THROW_NCML_PARSE_ERROR(_parser->getParseLineNumber(),
...
libdap::BaseType*
VariableElement::replaceArrayIfNeeded(NCMLParser& p, libdap::BaseType* pOrgVar,
		const string& name)
{
	VALID_PTR(pOrgVar);
	Array* pOrgArray = dynamic_cast<libdap::Array*>(pOrgVar);
	...
	auto_ptr<RenamedArrayWrapper> pNewArray = auto_ptr < RenamedArrayWrapper
			> (new RenamedArrayWrapper(
					dynamic_cast<Array*>(pOrgArray->ptr_duplicate())));
	p.deleteVariableAtCurrentScope(pOrgArray->name());
...
void VariableElement::addNewVariableAndEnterScope(NCMLParser& p,
		const std::string& dapType)
{
	...
	{
		THROW_NCML_PARSE_ERROR(_parser->getParseLineNumber(),
				"Cannot add a new scalar variable at current scope!  TypedScope="
						+ p.getTypedScopeString());
	}

	// Destroy it no matter what sicne add_var copies it
	auto_ptr<BaseType> pNewVar = MyBaseTypeFactory::makeVariable(dapType,
			_name);
	NCML_ASSERT_MSG(pNewVar.get(),
			"VariableElement::addNewVariable: factory failed to make a new variable of type: "
					+ dapType + " for element: " + toString());
...
bool VariableElement::isDimensionNumericConstant(
		const std::string& dimToken) const
{
	// for now just test the first character is a number and assume it's a number
	return isdigit(dimToken.at(0));
}

unsigned int VariableElement::getSizeForDimension(NCMLParser& p,
		const std::string& dimToken) const
{
	unsigned int dim = 0;
	...
	if (isDimensionNumericConstant(dimToken))
	{
		...
		{
			THROW_NCML_PARSE_ERROR(_parser->getParseLineNumber(),
					"Trying to get the dimension size in shape=" + _shape
							+ " for token " + dimToken
		...
		else
		{
			THROW_NCML_PARSE_ERROR(_parser->getParseLineNumber(),
					"Failed to find a dimension with name=" + dimToken
							...
							+ p.printAllDimensionsAtLexicalScope()
