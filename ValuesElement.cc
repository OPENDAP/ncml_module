...
namespace ncml_module
{
  ...
  ValuesElement::handleBegin()
  {
    ...
    BESDEBUG("ncml", "ValuesElement::handleBegin called with element=" << toString() <<
    ...
    if (!p.isScopeVariable())
      {
        THROW_NCML_PARSE_ERROR(_parser->getParseLineNumber(),
            "Got values element while not parsing a variable!  values=" + toString() +
    ...
    if (pVarElt->checkGotValues())
      {
        THROW_NCML_PARSE_ERROR(_parser->getParseLineNumber(),
            "Got a values element when one was already specified for this variable=" +
            pVarElt->toString() +
  ...
  ValuesElement::setScalarValue(libdap::BaseType& var, const string& valueAsToken)
  {
    ...
    if (sis.fail())
      {
        THROW_NCML_PARSE_ERROR(_parser->getParseLineNumber(),
            "Setting array values failed to read the value token properly!  value was for var name=" +
            var.name() + " and the value token was " + valueAsToken);
      }
  ...
  */
   ...
   */
   ...
   void
   ValuesElement::setVectorValues(libdap::Array* pArray, const vector<string>& valueTokens)
   {
     VALID_PTR(pArray);
   ...
   template<>
   void
   ValuesElement::setVectorValues <string> (libdap::Array* pArray, const vector<string>& valueTokens)
   {
     VALID_PTR(pArray);
   ...
   ValuesElement::parseAndSetCharValue(libdap::BaseType& var, const string& valueAsToken)
   {
     ...
     if (valueAsToken.size() != 1)
       {
         THROW_NCML_PARSE_ERROR(_parser->getParseLineNumber(),
             "Parsing scalar char, expected single character but didnt get it.  value was for var name=" +
             var.name() + " and the value token was " + valueAsToken);
       }
   ...
   void
   ValuesElement::parseAndSetCharValueArray(NCMLParser& /* p */, libdap::Array* pVecVar, const vector<string>& tokens)
   {
  ...
  ValuesElement::setVariableValuesFromTokens(NCMLParser& p, libdap::BaseType& var)
  {
    ...
    if (var.type() == dods_structure_c)
      {
        THROW_NCML_PARSE_ERROR(_parser->getParseLineNumber(),
            "Illegal to specify <values> element for a Structure type variable name=" + var.name() +
            " at scope=" + p.getScopeString());
      }
  ...
  void
  ValuesElement::setScalarVariableValuesFromTokens(NCMLParser& p, libdap::BaseType& var)
  {
  ...
  void
  ValuesElement::setVectorVariableValuesFromTokens(NCMLParser& p, libdap::BaseType& var)
  {
  ...
  template <typename DAPType>
  void
  ValuesElement::generateAndSetVectorValues(NCMLParser& p, libdap::Array* pArray)
  {
    ...
    DAPType start;
    {
      ...
      if (sis.fail())
        {
          THROW_NCML_PARSE_ERROR(_parser->getParseLineNumber(),
              "Failed to parse the values@start=" + _start + " for " + toString() +
              " at scope=" + p.getScopeString());
        }
    }
    ...
    DAPType increment;
    {
      ...
      if (sis.fail())
        {
          THROW_NCML_PARSE_ERROR(_parser->getParseLineNumber(),
              "Failed to parse the values@increment=" + _start + " for " + toString() +
              " at scope=" + p.getScopeString());
        }
    }
