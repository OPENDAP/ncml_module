...
namespace ncml_module
{
  ...
  ValuesElement::setScalarValue(libdap::BaseType& var, const string& valueAsToken)
  {
    ...
    if (sis.fail())
      {
        THROW_NCML_PARSE_ERROR("Setting array values failed to read the value token properly!  value was for var name=" +
  ...
  */
   ...
   */
   ...
   ValuesElement::parseAndSetCharValue(libdap::BaseType& var, const string& valueAsToken)
   {
     ...
     if (valueAsToken.size() != 1)
       {
         THROW_NCML_PARSE_ERROR("Parsing scalar char, expected single character but didnt get it.  value was for var name=" +
             var.name() + " and the value token was " + valueAsToken);
       }
  ...
  ValuesElement::setVariableValuesFromTokens(NCMLParser& p, libdap::BaseType& var)
  {
    ...
    if (var.is_simple_type())
      ...
      {
        THROW_NCML_PARSE_ERROR( "Can't call ValuesElement::setVariableValuesFromTokens for constructor type now!! "
  ...
  ValuesElement::generateAndSetVectorValues(NCMLParser& p, libdap::Array* pArray)
  {
    ...
    DAPType start;
    {
      ...
      if (sis.fail())
        {
          THROW_NCML_PARSE_ERROR("Failed to parse the values@start=" + _start + " for " + toString() +
              " at scope=" + p.getScopeString());
        }
    }
    ...
    DAPType increment;
    {
      ...
      if (sis.fail())
        {
          THROW_NCML_PARSE_ERROR("Failed to parse the values@increment=" + _start + " for " + toString() +
              " at scope=" + p.getScopeString());
        }
    }
  ...
  ValuesElement::autogenerateAndSetVariableValues(NCMLParser& p, BaseType& var)
  {
    ...
    case dods_byte_c:
      ...
      if (getNCMLTypeForVariable(p) == "char")
        {
          THROW_NCML_PARSE_ERROR("Can't use values@start for non-numeric values!");
        }
